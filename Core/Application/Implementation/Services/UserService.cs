...
namespace Visitor_Management_System.Core.Application.Implementation.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<BaseResponse<UserDto>> Delete(string Id)
        {
        ...
        public async Task<BaseResponse<UserDto>> Get(string email)
        {
            ...
            {
                UserId = users.Id,
                Action = $"Retrieve of {email} Details",
                Timestamp = DateTime.Now,
            };
            await _auditLog.CreateAsync(auditLog);
            await _auditLog.SaveAsync();
        ...
        public async Task<BaseResponse<ICollection<UserDto>>> GetAll()
        {
        ...
        public async Task<BaseResponse<ICollection<UserDto>>> GetAllHost()
        {
        ...
        public async Task<BaseResponse<ICollection<UserDto>>> GetAllSecurity()
        {
        ...
        public async Task<BaseResponse<UserDto>> RegisterAdmin(AdminProfileRequestModel model)
        {
            ...
            if (!IsValidEmail(model.Email))
            {
                ...
                {
                    Status = false,
                    Message = $"Invalid email format: {model.Email}.",
                };
            }
            var user = new User()
            {
        ...
        public async Task<BaseResponse<UserDto>> UserRegister(RegisterRequestModel model)
        {
            var host = _user.Check(x => x.Email == model.Email);
            if (host == true)
            {
        ...
        public async Task<BaseResponse<UserDto>> Update(string id, UpdateProfileRequestModel model)
        {
        ...
        private void SendRequestEmail(User user)
        {
            ...
            string message = $"<html><body><h2>Successful SignUp</h2></body></html>\n" +
                             ...
                             $"<html><body><h4>Password: {user.PassWord}</h4></body></html>";
