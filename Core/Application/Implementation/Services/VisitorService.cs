...
namespace Visitor_Management_System.Core.Application.Implementation.Services
{
    public class VisitorService : IVisitorService
    {
        ...
        public async Task<BaseResponse<VisitorDto>> Delete(string Id)
        {
            ...
            {
                UserId = visitors.Id,
                Action = $"This Visitor: {visitors.FirstName +""+ visitors.LastName} Info as been deleted",
        ...
        public async Task<BaseResponse<ICollection<VisitorDto>>> GetAll()
        {
            try
            {
                var visitors = await _visitor.GetAll();
        ...
        public async Task<BaseResponse<VisitorDto>> GetByEmail(string email)
        {
            ...
            {
                UserId = visit.Id,
                Action = $"Retrieving Visitor Name: {visit.FirstName +""+visit.LastName}",
                Timestamp = DateTime.Now,
            };
            await _auditLog.CreateAsync(auditLog);
            await _auditLog.SaveAsync();
        ...
        public async Task<BaseResponse<ICollection<VisitorDto>>> GetByHostEmail(string hostEmail)
        {
            var visit = await _visitor.GetAll(x => x.HostEmail == hostEmail);
            if (visit == null)
            {
                return new BaseResponse<ICollection<VisitorDto>>
                {
            ...
            var auditLog = new AuditLog
            {
                Action = $"Retrieving Visitors of {hostEmail}",
                Timestamp = DateTime.Now,
            };
            await _auditLog.CreateAsync(auditLog);
            await _auditLog.SaveAsync();
        ...
        public async Task<BaseResponse<ICollection<VisitDto>>> GetVisitByHostEmail(string hostEmail)
        {
            ...
            var approvedVisits = await _visit.GetAll(x => x.Visitor.HostEmail == hostEmail && x.VisitStatus == VisitStatus.Approved);
        ...
        public async Task<BaseResponse<VisitorDto>> Register(VisitorRequestModel model, VisitRequestModel visitModel)
        {
            ...
            var visit = new Visit
            {
                ...
                VisitTime = visitModel.VisitTime,
        ...
        public async Task<BaseResponse<VisitDto>> ApproveRequest(string visitId, string token)
        {
            var visit = await _visit.Get(visitId);
            ...
            var tokens = await _tokenService.ValidateAndRemoveToken(token);
            if (!tokens.Status)
            {
                return new BaseResponse<VisitDto>
                {
                    Status = false,
                    Message = "Invalid Visit Request Or Is been Used ",
                };
            }
        ...
        public async Task<BaseResponse<VisitDto>> DeniedRequest(string visitId, string token)
        {
            var visit = await _visit.Get(visitId);
...
<html>
    <body>
        ...
        <a href=""{approveUrl}"" style=""background-color: #4CAF50; /* Green */ color: white; padding: 10px; text-align: center; text-decoration: none; display: inline-block; border-radius: 5px;"">Approve</a>
        <a href=""{rejectUrl}"" style=""background-color: #f44336; /* Red */ color: white; padding: 10px; text-align: center; text-decoration: none; display: inline-block; border-radius: 5px; margin-left: 10px;"">Reject</a>
    </body>
</html>";
